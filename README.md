...
>
> [bhamiltoncx/go2rtc](https://github.com/bhamiltoncx/go2rtc) carries a set of changes for `nest:` (WebRTC) sources that are not yet in [upstream go2rtc](https://github.com/AlexxIT/go2rtc). If you do not use Nest cameras, use upstream. The `master` branch here is upstream `master` plus these commits; the changes are gated on the `nest/webrtc` producer so other sources are unaffected. Everything except the GOP cache and the snapshot-error wording has been sent upstream as draft PRs [#2491](https://github.com/AlexxIT/go2rtc/pull/2491), [#2493](https://github.com/AlexxIT/go2rtc/pull/2493), [#2494](https://github.com/AlexxIT/go2rtc/pull/2494), [#2495](https://github.com/AlexxIT/go2rtc/pull/2495) and [#2496](https://github.com/AlexxIT/go2rtc/pull/2496). Details and config examples are in [`internal/nest/README.md`](internal/nest/README.md#changes-in-this-fork).
>
> - **Streams no longer drop every 5 minutes.** Per-stream session keep-alive instead of one shared timer that never fired (upstream #2108, #2319). *Upstream PR [#2493](https://github.com/AlexxIT/go2rtc/pull/2493).*
> - **Periodic keyframes.** 2-second PLI+FIR keyframe requests for Nest, so consumers joining mid-GOP start in ~1 s instead of waiting out the camera's interval (upstream #2365, #2368). *Upstream PR [#2494](https://github.com/AlexxIT/go2rtc/pull/2494).*
> - **SPS/PPS in the RTSP SDP.** RTSP clients such as ffmpeg learn the video dimensions at DESCRIBE instead of needing a large `-probesize`. *Upstream PR [#2494](https://github.com/AlexxIT/go2rtc/pull/2494).*
> - **Stays under Google's rate limits.** A sliding-window throttler enforces the documented Device Access budgets (10 `executeCommand`/min per project, 5/min per device and command, 30/min and 100/h per camera, 5 device lists/min) before each request. A dial that would have to wait more than 5 s fails immediately so a fallback source takes over; keep-alives wait their turn. A real 429 holds the project off for a minute instead of the old 30 s/60 s sleep from upstream #1397, and a switched-off camera's 400 is remembered with an exponential hold-off (1 min doubling to 5 min, reset when it comes back) so polling it costs nothing. *Upstream PR [#2493](https://github.com/AlexxIT/go2rtc/pull/2493).*
> - **Cameras that are switched off fail fast.** A definitive 4xx from Google ("The camera is not available for streaming") returns immediately instead of after 30 s and 60 s retries, so a fallback source such as a placeholder image kicks in right away. *Upstream PR [#2493](https://github.com/AlexxIT/go2rtc/pull/2493).*
> - **Preloads stay on the primary source** and retry on a back-off if it cannot be dialed at startup, so a warm placeholder source never captures them; ordinary viewers still get the placeholder while a camera is off or briefly rate limited. *Upstream PR [#2496](https://github.com/AlexxIT/go2rtc/pull/2496).*
> - **Cold dials no longer hang.** A consumer that triggers the WebRTC dial no longer gets stuck on a receiver that never receives packets when Google sends on a different H264 payload type than the first one in its SDP answer (fixes `frame.jpeg` hanging on idle streams). *Upstream PR [#2495](https://github.com/AlexxIT/go2rtc/pull/2495).*
> - **No corrupt keyframes on attach.** Drops H264 FU-A fragments received before their start bit; fixes intermittent `frame.jpeg` "exit status 183" failures (upstream #2490, PR #2491, pion/rtp#370). *Upstream PR [#2491](https://github.com/AlexxIT/go2rtc/pull/2491); pion/rtp [#370](https://github.com/pion/rtp/issues/370).*
> - **Packet loss no longer produces smeared keyframes.** The H264 depacketizer watches RTP sequence numbers; on a gap it discards the partial access unit and passes nothing on until the next keyframe received in full (the H264 counterpart of upstream PR #2479 for H265). *Part of upstream PR [#2491](https://github.com/AlexxIT/go2rtc/pull/2491).*
> - **Readable snapshot errors.** ffmpeg's stderr and the NAL unit list are included when a keyframe transcode fails.
> - **GOP cache** (`#gop=1`). Rebase of upstream PR #1887 by seydx: consumers attaching mid-stream get the cached keyframe immediately. *Not resubmitted; see [#1887](https://github.com/AlexxIT/go2rtc/pull/1887).*
